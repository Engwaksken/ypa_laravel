...
<div class="dash-wrap">
    ...
    <div class="dash-head">
        ...
        <a href="{{ route('members.show', $m) }}" class="btn btn-outline-secondary">
    ...
    <form method="POST" action="{{ route('members.update', $m) }}">
            ...
            <div class="dash-panel-body">
                <div class="row g-3">
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="text" name="other_name" value="{{ $old['other_name'] ?? $m->other_name }}" class="form-control">
                    </div>
                    <div class="col-md-4">
                        <label class="form-label">Date of Birth <span class="text-danger">*</span></label>
                        <input type="date" name="date_of_birth" value="{{ $old['date_of_birth'] ?? ($m->date_of_birth ? $m->date_of_birth->format('Y-m-d') : '') }}" class="form-control @error('date_of_birth') is-invalid @enderror" required>
                    ...
                    <div class="col-md-4">
                        ...
                        <select name="sex" class="form-select @error('sex') is-invalid @enderror" required>
                            ...
                            @foreach($service::SEXES as $sex)
                                <option value="{{ $sex }}" @selected(($old['sex'] ?? $m->sex) === $sex)>{{ $sex }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="text" name="nationality" value="{{ $old['nationality'] ?? $m->nationality }}" class="form-control">
                    </div>
                    <div class="col-md-4">
                        <label class="form-label">NIN</label>
                        <input type="text" name="nin" value="{{ $old['nin'] ?? $m->nin }}" class="form-control @error('nin') is-invalid @enderror">
                        @error('nin')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>
                    <div class="col-md-4">
                        <label class="form-label">TIN Number</label>
                        <input type="text" name="tin_number" value="{{ $old['tin_number'] ?? $m->tin_number }}" class="form-control">
                    </div>
                    <div class="col-md-4">
        ...
        <div class="dash-panel mt-4">
            ...
            <div class="dash-panel-body">
                <div class="row g-3">
                    ...
                    <div class="col-md-6" id="source_station_wrap" style="{{ in_array($old['source'] ?? $m->source, ['Radio', 'TV']) ? '' : 'display:none;' }}">
            ...
            <div class="dash-panel-body">
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <input type="text" name="bank_account" value="{{ $old['bank_account'] ?? $m->bank_account }}" class="form-control">
                    </div>
                    <div class="col-md-6">
                        <label class="form-label">Account Name</label>
                        <input type="text" name="bank_account_name" value="{{ $old['bank_account_name'] ?? $m->bank_account_name }}" class="form-control">
                    </div>
                    <div class="col-md-6">
                        <label class="form-label">Bank Name</label>
                        <input type="text" name="bank_name" value="{{ $old['bank_name'] ?? $m->bank_name }}" class="form-control">
                    </div>
                    <div class="col-md-6">
                        <label class="form-label">Bank Branch</label>
                        <input type="text" name="bank_branch" value="{{ $old['bank_branch'] ?? $m->bank_branch }}" class="form-control">
                    </div>
                </div>
            </div>
        </div>
                    ...
                    <div class="col-md-4">
                        ...
                        <select name="branch_id" class="form-select @error('branch_id') is-invalid @enderror" required>
                            ...
                            @foreach($branches as $branch)
                                <option value="{{ $branch->id }}" @selected((int) ($old['branch_id'] ?? $m->branch_id) === (int) $branch->id)>{{ $branch->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-4">
                        ...
                        <select name="mobilizer_id" class="form-select @error('mobilizer_id') is-invalid @enderror" required>
                            ...
                            @foreach($mobilizers as $mobilizer)
                                <option value="{{ $mobilizer->id }}" @selected((int) ($old['mobilizer_id'] ?? $m->mobilizer_id) === (int) $mobilizer->id)>{{ $mobilizer->full_name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-4">
                        ...
                        <select name="membership_status" class="form-select @error('membership_status') is-invalid @enderror">
                            ...
                            @foreach($service::MEMBERSHIP_STATUSES as $ms)
                                <option value="{{ $ms }}" @selected(($old['membership_status'] ?? $m->membership_status) === $ms)>{{ $ms }}</option>
                            @endforeach
                        </select>
        ...
        <div class="d-flex gap-2 mt-4 mb-4">
            <button type="submit" class="btn btn-primary btn-lg"><i class="fas fa-save"></i> Update Member</button>
            <a href="{{ route('members.show', $m) }}" class="btn btn-outline-secondary btn-lg">Cancel</a>
        </div>
    </form>
...
(function () {
    ...
    const currentStation = @json($m->source_station);
    ...
    function renderStations() {
        ...
        if (source === 'Radio' || source === 'TV') {
            ...
            stationSelect.innerHTML = '<option value="">Select Station</option>' +
                stations.map(s => '<option value="' + s + '"' + (s === currentStation ? ' selected' : '') + '>' + s + '</option>').join('');
        } else {
